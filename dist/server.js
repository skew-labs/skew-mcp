...
// ---------------------------------------------------------------------------
let _skew = null;
let _readOnlySkew = null;
function buildSkewClient(wallet) {
    const require = createRequire(import.meta.url);
    const idl = require("@skew-labs/sdk/idl/skew_master.json");
...
async function getReadOnlySkewClient() {
    if (_skew)
        return _skew;
    if (_readOnlySkew)
    ...
    return {
        ...
        lane_selection: {
            ...
            auction_rfq_firm_tape: {
                purpose: "Buyer opens an on-chain competition window and MMs post firm quotes. finalize_rfq_auction publishes/takes the tape and refunds auction escrow; it is not by itself an option mint. Official Auction execution is forced through the Instant RFQ PM atomic-fill handoff.",
            ...
            pre_funded_legacy: {
                purpose: "Simple fully collateralized issuance using create_option/buy_option or the advanced-only quote-bound Auction bridge. It is useful for inventory/legacy primitives but does not show marginal PM capital efficiency.",
                tools: [
                    "skew_create_option",
                    "skew_buy_option",
...
function serializeJson(node) {
    ...
    if (typeof node === "object") {
        if (node instanceof PublicKey)
            return node.toBase58();
        if (Buffer.isBuffer(node))
            return Array.from(node);
        if (Array.isArray(node))
            return node.map(serializeJson);
        const obj = node;
...
const PRICING_DISCLAIMER = "Suggestion only. Not investment advice. The on-chain program does not read this number; settlement is governed solely by Pyth oracle data.";
// ---------------------------------------------------------------------------
...
async function fetchPovsState(underlying, conn) {
    ...
    if (idx === undefined)
        return null;
    const [pda] = PublicKey.findProgramAddressSync([POVS_STATE_SEED, Buffer.from([idx])], SKEW_PROGRAM_ID);
    const acc = await conn.getAccountInfo(pda);
    if (!acc || acc.data.length < POVS_OFFSET_REGIME_MICRO + 8)
        return null;
...
async function fetchSpot(underlying) {
    const feedId = HERMES_FEED_IDS[underlying];
    if (!feedId)
        throw new Error(`No Hermes feed for ${underlying}`);
...
async function fetchSurface(underlying, spot, strikes, expiries_days, iv) {
    const body = {
        underlying,
        spot,
        strikes,
...
function smileIv(strike, spot, atmIv) {
...
function classifyVolView(termStructure) {
...
function allowlistFairValue(input) {
    ...
    try {
        switch (name) {
            ...
            case "skew_get_iv_smile": {
                ...
                const atm = points.find((p) => Math.abs(p.strike_usd - spot) ===
            ...
            case "skew_get_term_structure": {
                ...
                const { price: spot } = await fetchSpot(underlying);
                const baselineIv = ASSET_BASELINE_IV[underlying] ?? 0.7;
                const raw = await fetchSurface(underlying, spot, [Math.round(spot)], TERM_LADDER_DAYS, baselineIv);
                const surface = applyHeuristicSurface(raw, spot, baselineIv);
                return ok(JSON.stringify({
                    underlying,
                    spot_usd: spot,
            ...
            case "skew_get_volatility_summary": {
                ...
                const raw = await fetchSurface(underlying, spot, [Math.round(spot * 0.85), Math.round(spot), Math.round(spot * 1.15)], [7, 30, 90], baselineIv);
                const surface = applyHeuristicSurface(raw, spot, baselineIv);
                const view = classifyVolView(surface.term_structure);
                // Staleness — Solana mainnet/devnet ~400ms slots.
                let last_update_minutes_ago = null;
                ...
                return ok(JSON.stringify({
                    ...
                    term_structure_summary: {
                        front_7d_atm_iv: Number((surface.term_structure.find((t) => t.expiry_days === 7)?.atm_iv ?? baselineIv).toFixed(4)),
                        mid_30d_atm_iv: Number(atm30d.toFixed(4)),
                        back_90d_atm_iv: Number((surface.term_structure.find((t) => t.expiry_days === 90)?.atm_iv ?? baselineIv).toFixed(4)),
                    },
                    risk_reversal_25d: Number(surface.skew["25d_risk_reversal"].toFixed(4)),
                    butterfly_25d: Number(surface.skew["25d_butterfly"].toFixed(4)),
                    vol_view: view.label,
            ...
            case "skew_get_vol_short": {
                ...
                if (!res.ok)
                    return err(`pricing /v1/vol/short ${res.status}`);
            ...
            case "skew_get_vol_long": {
                ...
                if (!res.ok)
                    return err(`pricing /v1/vol/long ${res.status}`);
                return ok(JSON.stringify(await res.json(), null, 2));
            }
            // -----------------------------------------------------------------------
            case "skew_get_vol_implied": {
                ...
                if (!res.ok)
                    return err(`pricing /v1/vol/iv ${res.status}`);
                return ok(JSON.stringify(await res.json(), null, 2));
            }
            // -----------------------------------------------------------------------
            case "skew_get_vol_premium": {
                ...
                if (a["fit_window"] != null)
                    body["fit_window"] = a["fit_window"];
                ...
                if (!res.ok)
                    return err(`pricing /v1/vol/vrp ${res.status}`);
                return ok(JSON.stringify(await res.json(), null, 2));
            }
            // -----------------------------------------------------------------------
            case "skew_get_margin_breakdown": {
                ...
                const regime = a["regime"] ?? "Calm";
                const tier = a["tier"] ?? "platinum";
            ...
            case "skew_estimate_fee": {
                ...
                const equityUsd = a["equity_usd"] != null ? Math.floor(Math.max(0, Number(a["equity_usd"]))) : 0;
                const verifiedTier = a["verified_tier"] ??
                    "standard";
                const side = a["side"] ?? "taker";
            ...
            case "skew_get_margin": {
                ...
                const spotUsd = explicitSpot != null ? Number(explicitSpot) : (await fetchSpot("BTC")).price;
            ...
            case "skew_upgrade_tier": {
                const skew = await getSkewClient();
                const r = await skew.upgradeTier(Number(a["target_rank"]));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            case "skew_downgrade_tier": {
                const skew = await getSkewClient();
                const r = await skew.downgradeTier(Number(a["target_rank"]));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_register_combo_intent": {
                const skew = await getSkewClient();
                const legsRaw = a["legs"];
                const legs = legsRaw.map((l) => ({
                    option: new PublicKey(String(l["option"])),
                    side: Number(l["side"]),
                    maxPremiumUsdc: Number(l["max_premium_usd"]),
                }));
                const r = await skew.registerComboIntent(BigInt(String(a["combo_id"])), legs, Number(a["total_max_premium_usd"]), BigInt(String(a["expiry_ts"])));
                return ok(JSON.stringify({
                    success: true,
            ...
            case "skew_cleanup_expired_combo_v2": {
                const skew = await getSkewClient();
                const r = await skew.cleanupExpiredComboV2(new PublicKey(String(a["buyer_authority"])), BigInt(String(a["combo_id"])));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_fetch_hamilton": {
                ...
                if (snap == null) {
                    return ok(JSON.stringify({ success: true, initialised: false, note: "HamiltonState PDA cold-start." }, null, 2));
                }
                return ok(JSON.stringify({
                    success: true,
                    initialised: true,
            ...
            case "skew_fetch_skew_metrics": {
                ...
                if (snap == null) {
                    return ok(JSON.stringify({ success: true, initialised: false, note: "SkewMetricsPda cold-start." }, null, 2));
                }
                return ok(JSON.stringify({
                    success: true,
                    initialised: true,
            ...
            case "skew_fetch_clearing_member": {
                ...
                return ok(JSON.stringify({
                    ...
                    equity_to_im_ratio: snap.lastImMicro > 0n
                        ? Number(snap.collateralMicro -
                            snap.ifContributionMicro -
                            snap.tierLockupCollateralMicro) / Number(snap.lastImMicro)
                        : null,
                    liq_trigger_active: snap.lastImMicro > 0n &&
                        Number(snap.collateralMicro - snap.ifContributionMicro - snap.tierLockupCollateralMicro) <
            ...
            case "skew_fetch_insurance_fund": {
                ...
                if (snap == null) {
                    return ok(JSON.stringify({ success: true, initialised: false, note: "InsuranceFund not initialised." }, null, 2));
                }
                return ok(JSON.stringify({
                    success: true,
                    initialised: true,
                    ...
                    total_capacity_usdc: Number(snap.tier3ProtocolSitgMicro +
                        snap.tier1MutualizedPoolMicro +
                        snap.tier2MutualizedPoolMicro +
                        snap.crossMutualizedPoolMicro) / 1e6,
            ...
            case "skew_fetch_isolated_vault": {
                ...
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "IsolatedVaultPda not initialised." }, null, 2));
            ...
            case "skew_fetch_combo_intent": {
                ...
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "ComboIntentPda not found / cleaned up." }, null, 2));
                return ok(JSON.stringify({ success: true, initialised: true, snapshot: serializeJson(snap) }, null, 2));
            }
            case "skew_fetch_cross_asset": {
                const skew = await getReadOnlySkewClient();
                const program = skew._program();
                const accs = await program.account["crossAssetMatrix"].all();
                if (accs.length === 0)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "CrossAssetMatrix not initialised." }, null, 2));
                return ok(JSON.stringify({
                    success: true,
                    initialised: true,
            ...
            case "skew_fetch_series_listing": {
                ...
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "SeriesListingPda not found." }, null, 2));
                return ok(JSON.stringify({ success: true, initialised: true, snapshot: serializeJson(snap) }, null, 2));
            }
            case "skew_fetch_builder_code": {
                ...
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "BuilderCodePda not found." }, null, 2));
                return ok(JSON.stringify({ success: true, initialised: true, snapshot: serializeJson(snap) }, null, 2));
            }
            case "skew_fetch_conditional_order": {
                const skew = await getReadOnlySkewClient();
                const snap = await skew.fetchConditionalOrder(new PublicKey(String(a["authority"])), BigInt(String(a["order_id"])));
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "ConditionalOrderPda not found." }, null, 2));
                return ok(JSON.stringify({ success: true, initialised: true, snapshot: serializeJson(snap) }, null, 2));
            }
            ...
            case "skew_fetch_combo_intent_v2": {
                const skew = await getReadOnlySkewClient();
                const snap = await skew.fetchComboIntentV2(new PublicKey(String(a["buyer"])), BigInt(String(a["combo_id"])));
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "ComboIntentPdaV2 not found." }, null, 2));
                return ok(JSON.stringify({ success: true, initialised: true, snapshot: serializeJson(snap) }, null, 2));
            }
            // Lifecycle ───────────────────────────────────────────────────────────
            ...
            case "skew_call_variation_margin": {
                const skew = await getSkewClient();
                const remaining = (a["remaining_accounts"] ?? []).map((s) => new PublicKey(s));
                const r = await skew.callVariationMargin(new PublicKey(String(a["cm_authority"])), remaining);
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            case "skew_register_clearing_member": {
                ...
                return ok(JSON.stringify({ success: true, cm_pda: r.cmPda.toBase58(), tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_init_fee_config": {
                ...
                return ok(JSON.stringify({ success: true, fee_config: r.feeConfig.toBase58(), tx_signature: r.txSignature }, null, 2));
            }
            case "skew_init_collateral_policy": {
                const skew = await getSkewClient();
                const r = await skew.initCollateralPolicy();
                return ok(JSON.stringify({ success: true, policy: r.policy.toBase58(), tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_register_rfq_maker": {
                ...
                return ok(JSON.stringify({ success: true, registry_pda: r.registry.toBase58(), tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_fetch_native_sol_vault": {
                ...
                if (!userArg)
                    return err("user pubkey required for read-only NativeSolVault fetch");
                const user = new PublicKey(String(userArg));
                const snap = await skew.fetchNativeSolVault(user);
                if (snap == null)
                    return ok(JSON.stringify({ success: true, initialised: false, note: "NativeSolVault not yet initialised." }, null, 2));
                return ok(JSON.stringify({
                    success: true,
                    initialised: true,
                    user: user.toBase58(),
            ...
            case "skew_cleanup_expired_conditional_order": {
                const skew = await getSkewClient();
                const r = await skew.cleanupExpiredConditionalOrder(new PublicKey(String(a["order_authority"])), BigInt(String(a["order_id"])));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            case "skew_apply_close_isolated_action": {
                const skew = await getSkewClient();
                const r = await skew.applyCloseIsolatedAction(BigInt(String(a["order_id"])), new PublicKey(String(a["action_target"])));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_finalize_combo_leg_v2": {
                const skew = await getSkewClient();
                const r = await skew.finalizeComboLegV2(new PublicKey(String(a["intent"])), Number(a["leg_index"]), BigInt(Math.round(Number(a["realised_premium_usd"]) * 1e6)));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_withdraw_builder_fees": {
                const skew = await getSkewClient();
                const r = await skew.withdrawBuilderFees(BigInt(Math.round(Number(a["amount_usdc"]) * 1e6)));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_list_series": {
                ...
                const r = await skew.listSeries({
                    ...
                    optionTypeName: String(a["option_type"]),
                    direction: Number(a["direction"]),
                });
                return ok(JSON.stringify({ success: true, series_pda: r.series.toBase58(), tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_governance_set_series_max_oi": {
                const skew = await getSkewClient();
                const r = await skew.governanceSetSeriesMaxOi(new PublicKey(String(a["series"])), Number(a["max_oi_count"]));
                return ok(JSON.stringify({ success: true, tx_signature: r.txSignature }, null, 2));
            }
            ...
            case "skew_get_settlement_payoff": {
                ...
                if (!res.ok)
                    return err(`pricing /settlement_payoff returned ${res.status}`);
            ...
            case "skew_get_dvol_replication": {
                ...
                if (!res.ok)
                    return err(`pricing /dvol_replication returned ${res.status}`);
                return ok(await res.text());
            }
            case "skew_get_combo_quote": {
                ...
                if (!res.ok)
                    return err(`pricing /combo_quote returned ${res.status}`);
                return ok(await res.text());
            }
            case "skew_get_recovery_priority": {
                ...
                if (!res.ok)
                    return err(`pricing /recovery_priority returned ${res.status}`);
                return ok(await res.text());
            }
            case "skew_get_if_replenish_check": {
                ...
                if (!res.ok)
                    return err(`pricing /if_replenish_check returned ${res.status}`);
                return ok(await res.text());
            }
            // -----------------------------------------------------------------------
            default:

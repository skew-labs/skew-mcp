...
const DISABLED_TOOL_REASONS: Record<string, string> = {
  skew_apply_early_exercise_action:
    "On-chain early-exercise automation is disabled until the direct exercise CPI ships. This MCP tool is hidden and stale calls fail closed instead of emitting a state-only event.",
  skew_apply_sell_via_rfq_action:
    "On-chain sell-via-RFQ automation is disabled until the direct RFQ CPI ships. Use the normal Auction RFQ flow instead.",
  skew_apply_buyback_via_rfq_action:
    "On-chain buyback-via-RFQ automation is disabled until the direct RFQ CPI ships. Use the normal Auction RFQ flow instead.",
  skew_take_best_quote:
    "Auction RFQ take_best_quote is hidden at launch because it does not complete option mint/close semantics. Use finalize_rfq_auction for firm tape/refund and Instant RFQ atomic_fill_from_relay for cleared execution.",
};

const DISABLED_TOOL_NAMES = new Set(Object.keys(DISABLED_TOOL_REASONS));

const DEPRECATED_TOOL_NAMES = new Set<string>([...DISABLED_TOOL_NAMES]);
const READ_PREFIX_WRITE_EXCEPTIONS = new Set<string>([
  "skew_get_margin",
  "skew_list_series",
]);

export function getSkewDisabledToolReason(name: string): string | null {
  return DISABLED_TOOL_REASONS[name] ?? null;
}

export function isSkewReadOnlyTool(name: string): boolean {
  if (READ_PREFIX_WRITE_EXCEPTIONS.has(name)) return false;
  return (
...
export const SKEW_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      "Buy an existing option on Skew devnet. Pays premium from buyer's USDC ATA to the creator. For Auction RFQ execution, prefer skew_buy_option_from_rfq_quote so the buyer action is bound to the best firm quote.",
    inputSchema: {
      type: "object",
      properties: {
        option_address: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        allow_expired_quote: {
          type: "boolean",
          description:
            "Default false. If false, rejects when the best quote's valid_until_slot has passed.",
        },
        require_best_quote_for_maker: {
          type: "boolean",
          description:
            "Default true. If true, the configured MCP wallet must equal auction.best_quote_mm.",
        },
        dry_run: {
          type: "boolean",
          description:
            "If true, simulate create+deposit and return logs/CU without sending transactions.",
        },
      },
      required: ["auction"],
    },
  ...
  },
  {
    ...
    description:
      "Buyer-side Instant RFQ quote request. Broadcasts a relay-backed RFQ using the configured wallet as buyer and returns relay_nonce plus live CM/MM quote acknowledgements. This is the PM/CM atomic-fill lane; use skew_hit_instant_rfq_quote to execute one returned quote.",
    inputSchema: {
      type: "object",
      properties: {
        underlying: {
        ...
        timeout_ms: {
          ...
          maximum: 600000,
          description:
            "How long to collect quote_ack messages before returning. Defaults to 60000; max 10 minutes for demo desks and human approval flows.",
        },
        max_quotes: {
          type: "integer",
          minimum: 1,
        ...
        required_cm_pubkey: {
          type: "string",
          description:
            "Optional maker/CM pubkey filter. When set, only quote_ack messages from this CM are returned and the hit template is pinned to that CM.",
        },
        relay_url: {
          type: "string",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_premium_micro: {
          type: "string",
          description:
            "Optional override for Instant RFQ premium cap in USDC micro-units. Defaults to the auction best quote premium when present, otherwise auction max premium.",
        },
        settlement_mint: {
          type: "string",
          description:
            "Optional settlement mint override. Defaults to USDC for non-inverse Auction RFQs.",
        },
        timeout_ms: {
          type: "integer",
          minimum: 500,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        premium_micro: {
          type: "string",
          description:
            "Selected premium in USDC micro-units, usually copied from the quote_ack. Required unless premium_usd is supplied.",
        },
        premium_usd: {
          type: "number",
          description:
            "Alternative selected premium in USD. Required only when premium_micro is omitted.",
        },
        underlying: {
          type: "string",
          enum: UNDERLYING_ENUM,
        ...
        quote_expiry_seconds: {
          ...
          maximum: 600,
          description:
            "Digest validity horizon from now. Defaults to 600 seconds for MCP-operated fills.",
        },
        timeout_ms: {
          type: "integer",
          minimum: 5000,
          maximum: 600000,
          description:
            "How long to wait for buyer_tx_request/fill_executed. Defaults to 120000; max 10 minutes.",
        },
        relay_url: {
          type: "string",
          description: "Optional relay WebSocket URL. Defaults to the Skew devnet relay.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_premium_micro: {
          type: "string",
          description:
            "Optional premium-cap override used only to rebuild/display the derived Instant RFQ request.",
        },
        settlement_mint: {
          type: "string",
          description:
    ...
    inputSchema: {
      ...
      properties: {
        premium_micro: {
          type: "string",
          description:
            "Fixed quote premium in USDC micro-units. Required unless premium_usd is supplied. Use a small value for devnet demos.",
        },
        premium_usd: {
          type: "number",
          description:
            "Alternative fixed quote premium in USD. Required only when premium_micro is omitted.",
        },
        quote_ttl_seconds: {
          type: "integer",
          minimum: 5,
        ...
        timeout_ms: {
          ...
          maximum: 600000,
          description:
            "How long to wait for quote request and fill completion. Defaults to 600000.",
        },
        auto_prepare: {
          type: "boolean",
  ...
  },
  {
    ...
    description:
      "Read a CM's on-chain PM cache sidecar and return whether cached margin / cached fill can be used. Reports dirty/stale/registry mismatch blockers explicitly. Read-only.",
    inputSchema: {
      type: "object",
      properties: {
        cm_authority: {
          type: "string",
          description:
            "CM authority wallet. Defaults to the configured MCP write wallet if omitted.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "Preview the PM cache envelope for a candidate fill. This is read-only and not an execution guarantee; exact post-fill IM is still produced by relay/SDK transaction preflight.",
    inputSchema: {
      type: "object",
      properties: {
        cm_authority: {
        ...
        estimated_post_im_micro: {
          type: "string",
          description:
            "Optional post-fill IM estimate in USDC micro-units. If omitted, the tool returns cache status only.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "List owner-first rent reclaim candidates for a wallet. Returns close blockers instead of pretending unsafe accounts can be closed. Read-only.",
    inputSchema: {
      type: "object",
      properties: {
        authority: {
          type: "string",
          description:
            "Wallet whose reclaim queue should be scanned. Defaults to the configured MCP write wallet if omitted.",
        },
      },
      required: [],
    },
  ...
  },
  {
    ...
    description:
      "Attempt Dutch-auction liquidation against a PM-issued option for a defaulting Clearing Member. Permissionless, but guarded: the liquidator must differ from the defaulting CM and healthy CMs are rejected by the on-chain liquidation trigger.",
    inputSchema: {
      type: "object",
      properties: {
        option_address: {
  ...
  },
  {
    ...
    description:
      "Run the full PM walk and refresh the CM risk-cache sidecar. Requires a configured write keypair and the CM's registry-tracked option accounts.",
    inputSchema: {
      type: "object",
      properties: {
        current_spot_usd: {
  ...
  },
  {
    ...
    description:
      "Prepare an unsigned owner-first rent reclaim batch for wallet signing. The MCP server does not close accounts server-side.",
    inputSchema: {
      type: "object",
      properties: {
        authority: {
          type: "string",
          description:
            "Wallet whose reclaimable accounts should be prepared. Defaults to the configured MCP write wallet if omitted.",
        },
      },
      required: [],
    },
  ...
  // ──────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      "Return Skew's supported assets, 11 payoff names, collateral rails, trade lanes, profile counts, and MCP agentGuide. Call this immediately after skew_get_signer_info so the agent chooses Instant RFQ vs Auction RFQ vs secondary vs pre-funded issuance correctly.",
    inputSchema: {
      type: "object",
      properties: {},
      required: [],
    ...
    inputSchema: {
      ...
      properties: {
        ...
        filter_option_pda: {
          type: "string",
          description:
            "Fetch a single option PDA directly. Use this for receipt/readback after a known tx instead of scanning the whole market.",
        },
        holder: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Read a wallet's live Skew portfolio directly from on-chain OptionAccount PDAs. Long side = current holder; short side = creator/writer. Also includes Clearing Member state when registered. Use this after create/buy/transfer to prove the option is visible, not just minted.",
    inputSchema: {
      type: "object",
      properties: {
        owner: {
          type: "string",
          description:
            "Wallet pubkey base58. Defaults to configured MCP write wallet. Required when the MCP server is running read-only without a write keypair.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "List live Auction RFQs from the same public RFQ tape used by the terminal. This discovers auction PDAs before you know their address by merging the indexer view with a bounded on-chain snapshot. Use this before `skew_fetch_rfq_auction`, quote submission, or finalize workflows.",
    inputSchema: {
      type: "object",
      properties: {
        limit: {
        ...
        source: {
          ...
          description:
            "Optional source override. Omit for terminal-equivalent indexer plus on-chain snapshot.",
        },
      },
      required: [],
    },
  },
  {
    ...
    description:
      "List the public secondary tape from the same listing endpoint used by the terminal. This is discovery/readback only, not an escrowed orderbook. Payment uses skew_buy_secondary_listing and delivery is complete only after seller-side skew_transfer_option returns readback_ok=true.",
    inputSchema: {
      type: "object",
      properties: {
        limit: {
        ...
        pending: {
          type: "boolean",
          description:
            "Return listings that are still pending off-chain settlement/indexer confirmation.",
        },
        seller: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Post a seller-signed secondary-market discovery row for an option the configured MCP wallet currently holds. This does not escrow or lock the option. Actual delivery is explicit seller-side skew_transfer_option after a buyer pays or accepts terms.",
    inputSchema: {
      type: "object",
      properties: {
        option_address: {
  ...
  },
  {
    ...
    description:
      "Compute the off-chain clearing-class portfolio-margin breakdown for a candidate options portfolio via the skew-pricing /margin_breakdown endpoint. Launch PM policy (2026-05-08): conservative ConvexHullIM/floor stack, calendar credit disabled, non-vanilla Greek credit audit-gated, and ICC credit applied once under a capped policy. Pass `tier` as the compatibility value: standard=M0 Segregated, silver=M1 Portfolio, gold=M2 Cross-Asset, platinum=M3 Clearing Prime. The response returns a full class ladder for transparency. Advisory-only — the on-chain calculate_margin handler remains the final risk check.",
    inputSchema: {
      type: "object",
      properties: {
        legs: {
        ...
        regime: {
          ...
          enum: ["Calm", "Stress"],
          description:
            "Regime to drive the per-asset shock and ICC rho matrix. Default Calm. M3 returns both calm and stress IMs side-by-side; lower classes omit regime fields.",
        },
        tier: {
          type: "string",
          enum: ["standard", "silver", "gold", "platinum"],
          description:
            "Clearing-class compatibility value. Default `platinum` maps to M3 Clearing Prime. Lockup schedule: M0 $0, M1 $500K, M2 $2M, M3 $10M (30-day lockup). The full ladder under all 4 classes is returned in `tier_ladder_im_usd` for backward-compatible clients.",
        },
      },
      required: ["legs"],
    },
  ...
  },
  {
    ...
    description:
      "Step the CM down to a lower clearing class. Releases class-specific USDC lockup back to free_collateral. Requires now >= tier_locked_until (30 days after most recent upgrade). Strict rank decrease only.",
    inputSchema: {
      type: "object",
      properties: {
        target_rank: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        assetIdx: {
          ...
          maximum: 4,
          description:
            "Numeric asset enum: BTC=0, ETH=1, SOL=2, XRP=3, HYPE=4. Required unless a symbol selector is supplied.",
        },
        asset_idx: {
          type: "integer",
          minimum: 0,
  ...
  },
  {
    ...
    description:
      "Read singleton CrossAssetMatrix — 10 pairwise correlations + 10 stress-conditional correlations across BTC/ETH/SOL/XRP/HYPE. Drives ICC adjustment for M2/M3 clearing-class portfolio margin.",
    inputSchema: { type: "object", properties: {} },
  },
  {
  ...
  },
  {
    ...
    description:
      "Read the full quote-depth tape for one RFQ auction PDA from the public web/indexer API. This is read-only and complements skew_fetch_rfq_auction, which only exposes the on-chain best quote snapshot.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        option_address: {
          type: "string",
          description:
            "Alias for option. Required unless option is supplied; accepted for consistency with skew_buy_option output.",
        },
  ...
  },
  {
    ...
    description:
      "Register an Active option currently held by the configured wallet into that wallet's Clearing Member PositionRegistry as a PM long hedge. Use this after a CM buys or receives a long option so calculate_margin and future atomic_fill_from_relay checks can see the hedge. This mutates the CM registry; it is not needed for ordinary non-CM retail holds.",
    inputSchema: {
      type: "object",
      properties: {
        option: {
  ...
  },
  {
    ...
    description:
      "Remove an option held by the configured wallet from that wallet's PM PositionRegistry before transfer/close workflows. The wallet must still be the current holder.",
    inputSchema: {
      type: "object",
      properties: {
        option: {
  ...
  },
  {
    ...
    description:
      "Recompute PM for the option writer and release excess marginal IM from the option escrow back into the writer CM escrow. Use after hedges are tracked or after calculate_margin shows the book needs less escrow. max_release_usdc=0 releases all eligible excess.",
    inputSchema: {
      type: "object",
      properties: {
        option: {
  ...
  },
  {
    ...
    description:
      "One-time onboarding: enrolls the caller as a Clearing Member. Initialises the ClearingMemberPda + USDC escrow. Required before opening any cross-margined position. Starts in the M0 Segregated clearing class (no class floor, but a non-zero seed is recommended).",
    inputSchema: {
      type: "object",
      properties: {
        initial_collateral_usdc: {
  ...
  // ──────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      "One-time enrollment: register the caller as an RFQ market maker. Locks the maker-deposit in SOL for quote slashing/rent. The wallet needs at least 1.02 SOL before this call.",
    inputSchema: { type: "object", properties: {} },
  },
  {
  ...
  },
  {
    ...
    description:
      "Browser/operator-wallet RFQ quote lane. The caller's wallet signs the transaction directly; no detached signMessage/Ed25519 signature is required. Use this for terminal MM quote submissions.",
    inputSchema: {
      type: "object",
      properties: {
        auction: { type: "string", description: "Auction PDA base58." },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        buyer_usdc_ata: {
          type: "string",
          description:
            "Optional buyer USDC ATA pubkey base58. If omitted, SDK derives it from the auction buyer and configured USDC mint.",
        },
      },
      required: ["auction"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        volume_30d_usd: {
          type: "number",
          description:
            "Caller's 30-day rolling options notional in USD. Drives VIP volume discount resolution.",
        },
        equity_usd: {
          type: "number",
          description:
        ...
        verified_tier: {
          ...
          enum: ["standard", "silver", "gold", "platinum"],
          description:
            "Clearing-class compatibility value. Adds 0/10/20/30% taker discount on top of VIP. Maker fee is unaffected by clearing class. Default `standard` = M0.",
        },
        side: {
          type: "string",
          enum: ["taker", "maker"],
          description:
            "Trade side. Default `taker`. Only takers benefit from clearing-class discount.",
        },
        premium_usd: {
          type: "number",
          description:
...

  {
    ...
    description:
      "Buyer accepts the current best firm quote on an Auction RFQ. This refunds unused max-premium escrow and marks the auction tape as taken. For fully cleared PM/CM option minting, use the Instant RFQ atomic-fill lane.",
    inputSchema: {
      type: "object",
      properties: {
        auction_pda: {
        ...
        expected_premium_micro: {
          type: "string",
          description:
            "Optional exact USDC-micro premium bigint string. If supplied, overrides expected_premium_usd.",
        },
        via_relay: {
          type: "boolean",
          description:
            "Deprecated no-op. Relay-signed take-best-quote remains disabled because only the buyer wallet can sign this direct auction accept.",
        },
      },
...

  {
    ...
    description:
      "Refresh the caller/MM's current best quote before close_slot. Requires a detached ed25519 signature over rfq_quote_digest, just like skew_submit_rfq_quote.",
    inputSchema: {
      type: "object",
      properties: {
        auction_pda: {
...

  {
    ...
    description:
      "Publish a live on-chain MakerAxe inventory-intent entry. This is an MM discovery primitive, not settlement; traders use axes to route RFQs to likely counterparties.",
    inputSchema: {
      type: "object",
      properties: {
        axe_id: {
...

  {
    ...
    description:
      "Update a live on-chain MakerAxe inventory-intent entry. Owner/MM wallet only; pass the same mutable band, size, side, and validity fields used by publish.",
    inputSchema: {
      type: "object",
      properties: {
...

  {
    ...
    description:
      "Revoke/close a live MakerAxe entry. Rent returns to the MM wallet.",
    inputSchema: {
      type: "object",
      properties: {

...
export const SKEW_TOOLS: Tool[] = [
  ...
  // ──────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      `Create a pre-funded option listing on Skew (Solana devnet). Deposits USDC collateral and mints an option SPL token. Returns the option PDA address. Supported assets: ${UNDERLYING_ENUM.join(", ")}. Payoffs: ${PAYOFF_ENUM.join(", ")}.`,
    inputSchema: {
      type: "object",
      properties: {
        underlying: {
  ...
  },
  {
    ...
    description:
      "Register a stop-loss / take-profit / OCO conditional order gated on Pyth EMA. Permissionless keeper triggers via execute_conditional_order when oracle crosses. Then the user fires apply_*_action (close_isolated, sell_via_rfq, buyback_via_rfq, early_exercise) to perform the settlement. Phase 1633.D Drift v2 trigger pattern.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Read on-chain DvolPda for an asset — 28d / 90d DVOL variance index + realized variance. Used by combo_v2 quoting + variance-swap replication.",
    inputSchema: {
      type: "object",
      properties: { underlying: { type: "string", enum: ["BTC", "ETH", "SOL", "XRP", "HYPE"] } },
      required: ["underlying"],
  ...
  },
  {
    ...
    description:
      "Permissionless keeper trigger — flips an Active conditional order's status to Triggered when the Pyth EMA crosses past grace_slots. Then user calls apply_*_action.",
    inputSchema: {
      type: "object",
      properties: {
...
export function getSkewMcpProfile(raw: string | undefined): SkewMcpProfile {
  if (raw === "advanced" || raw === "governance" || raw === "all") return raw;
  return "core";
}
